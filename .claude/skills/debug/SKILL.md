...
The system passes authentication variables (`CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`) via stdin to keep them out of process listings. Other env vars are not exposed.

To verify the container can run:
```bash
...
echo -e "\n3. Docker running?"
docker info &>/dev/null && echo "OK" || echo "NOT RUNNING - start Docker Desktop or run: sudo systemctl start docker"
